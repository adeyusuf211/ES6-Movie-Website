...
// Menampilkan data
const button = document.querySelector('.cari-film');
button.addEventListener('click', async function() {
    const inputButton = document.querySelector('.form-control');
    const movies      = await getMovies(inputButton.value);
    ubahTampilan(movies);
});

function getMovies(keyword) {
    return fetch('http://www.omdbapi.com/?apikey=95281255&s=' + keyword)
    .then(response => response.json())
    .then(response => response.Search);
}

function ubahTampilan(movies) {
    let cards    = '';
...
document.addEventListener('click', async function(e) {
    if(e.target.classList.containts('modal-button')) {
        const imdbID        = e.target.dataset.imdbid;
        const movieDetail   = await getMovieDetail(imdbID);
        tampilanDetail(movieDetail);
    }
});

function getMovieDetail(imdbid) {
    return fetch('http://www.omdbapi.com/?apikey=95281255&s=' + imdbID)
            .then(response => response.json())
            .then(m => m);
}

function tampilanDetail(m) {
...
function showMovie(m) {
    return `<div class="col-md-4 my-3">
                <div class="card">
                    ...
                    <a href="#" class="btn btn-primary modal-button" data-bs-toggle="modal" data-bs-target="#movieDetail" data-imdbid="${m.imdbID}">Show Detail</a>
                    </div>
                </div>
            </div>`;
}

function moviesDetail(m) {
